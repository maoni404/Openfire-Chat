...
public class EmailListener {
    ...
    private void listenMessage(Message message) throws Exception {
        ...
        if (fromUser != null && userCollection.size() > 0 )
        {
            ...
            for (String username: userCollection)
            {
                ...
                processMeeting(meetingJSON, username, bookmark.getProperty("url"));
    ...
    private void configureRoom(MUCRoom room, String title, String owner)
    {
        ...
        org.dom4j.Element element = iq.setChildElement("query", "http://jabber.org/protocol/muc#owner");
    ...
    private void sendWorkgroupEmail(String email, String workgroupNodeName, String template, String room)
    {
        ...
        sendEmail(email, email, title, replaceTokens(template, variables), null);
    }
            ...
            if (userid != null)
            {
                ...
                if (messageBody != null)
                {
                    ...
                    if (globalConnections.containsKey(userid) == false)
                    {
                        ...
                        .setResource("ofchat" + new Random(new java.util.Date().getTime()).nextInt())
                        ...
                        connection.connect();
                        connection.login(userid, null);
                        globalConnections.put(userid, connection);
                    }

                    XMPPTCPConnection globalConnection = globalConnections.get(userid);
                    ...
                    if (isOnline(workgroupName, globalConnection))
                    {
                        ...
                        if (workgroup != null) {
                            try {
                                workgroup.joinQueue(details, JidCreate.entityBareFrom(userid + "@" + XMPPServer.getInstance().getServerInfo().getXMPPDomain()));
                            }
    ...
    private boolean isOnline(final String workgroupName, XMPPTCPConnection globalConnection)
    {
    ...
    private Workgroup getWorkgroup(final String workgroupName, final XMPPTCPConnection globalConnection)
    {
        ...
        {
            try {
                workgroup = new Workgroup(JidCreate.entityBareFrom(workgroupName), globalConnection);
                ...
                workgroup.addInvitationListener(new WorkgroupInvitationListener()
                {
                    public void invitationReceived(WorkgroupInvitation workgroupInvitation)
                    {
                        ...
                        String userid = globalConnection.getUser().toString();
    ...
    public void processMeeting(JSONObject meeting, String username, String videourl)
    {
        ...
        try {
            ...
            java.util.Date start = new java.util.Date(meeting.getLong("startTime"));
            java.util.Date end = new java.util.Date(meeting.getLong("endTime"));
